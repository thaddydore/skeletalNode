...
workers.performCheck = function (originalCheckData) {
  ...
  moduleToUse.request(requestDetail, function (res) {
...
workers.processCheckOut = function (originalCheckData, checkOutcome) {
  ...
  //save the update
  _date.update('checks', newCheckedData.id, newCheckedData, (error, checkData) => {
    if (!error && checkData) {
...
workers.alertUserstatusChange = function (newCheckedData) {
  ...
  helpers.sendTwilloSms(checkData.userPhone, msg, (error) => {
    if (!error) {
      console.log('Success: user was alerted to a status change in check via sms', sms);
    } else {
