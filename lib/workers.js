      ...
      checks.forEach(check => {
        _data.read('checks', check, (error, originalCheckData) => {
          if (!error && originalCheckData) {
            ...
            debug('Error reading one the checks data');
          }
        });
      });
    } else {
...
workers.alertUsersStatusChange = function (newCheckedData) {
  ...
  helpers.sendTwilloSms(newCheckedData.userPhone, msg, (error) => {
    if (!error) {
      debug('Success: user was alerted to a status change in check via sms', msg);
    } else {
      debug('Error : could not send to user has a state change in their check');
    }
  })
};
      ...
      logs.forEach(logName => {
        ...
        _log.compress(logId, newFileId, (error) => {
          if (!error) {
            ...
            debug("Error compressing of the log files", error);
          }
        });
      })
    } else {
